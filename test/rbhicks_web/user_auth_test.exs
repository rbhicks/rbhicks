defmodule RbhicksWeb.UserAuthTest do
  use RbhicksWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Rbhicks.Accounts
  alias Rbhicks.Accounts.Scope
  alias RbhicksWeb.UserAuth

  import Rbhicks.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RbhicksWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      RbhicksWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        RbhicksWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
